...
class Option: 
    ...
    def payoffobject(self, S:float, in_moneyness:bool=True)->PayOffObject2:
        K = self.specification.strike
        Bu = self.specification.barrier_up
        Bd = self.specification.barrier_down
        G = self.specification.gap_trigger
        Ba = self.specification.binary_amout
        R = self.specification.rebate
        if in_moneyness: 
            K,Bu,Bd,G,Ba,R = K/S,Bu/S,Bd/S,G/S,Ba/S,R/S
        return PayOffObject2(K,Bu,Bd,G,Ba,R)
